...
void updateXbee() {                                 //The main function to call during loop() after code integration. This manages incoming radio traffic and contacts
  ...
  if(((millis()/1000.0)-downtimeA)>waitForReconnection && connectedA != 0x00){
    ...
    connectedA = 0x00;
    searching = true;
    for(int i=0;i<CDU_RX_SIZE;i++) recoveredDataA[i]=0x00; //Clears data so it's obvious a disconnection has occurred
  }
    ...
    connectedB = 0x00;
    searching = true;
    for(int i=0;i<CDU_RX_SIZE;i++) recoveredDataB[i]=0x00; //Clears data so it's obvious a disconnection has occurred
  }
