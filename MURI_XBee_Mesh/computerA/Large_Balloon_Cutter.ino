//============================================================================================================================================
// MURI Resistor Cutter Box A (for Large Balloon)
...
#define CUT_INTERVAL 2*M2MS              // the cutting mechanism will stay on for 2 minutes


// Fix statuses
#define NOFIX 0x00 // indicate GPS does not have a fix
#define FIX 0x01   // indicates GPS does have a fix
...
uint8_t tempCounter = 0, battCounter = 0, boundCounter = 0, timerCounter = 0, initCounter = 0;
...
uint8_t cutReasonA = 0x22; // 0x22 indicates no cut
uint8_t cutStatusA = 0x01; // 1 for false (no cut), 2 for true (cut)
bool cutterOnA = false; // variable to activate resistor cut
...
struct GPSData{ // proposed struct filled out by updateTelemetry
  float alt;
  float latitude;
  float longitude;
  float AR;
...
struct data{                                 // FOR OUTGOING DATA (data sent to gondola and logged on SD)
  uint8_t startByte;
  uint8_t cutterTag;
...
struct input{                                // FOR INCOMING DATA (to receive cut commands)
  uint8_t startByte;
  uint8_t cutterTag;
  uint8_t fixStatus;
  uint8_t command;
...
void loop() {
  ...
  if(millis() - updateStamp > UPDATE_INTERVAL) {   // does the following every update interval
    ...
    updateTelemetry();  // update GPS data (output for decision making)
    ...
    updateXbee();       // gets data from Gondola
     ...
     if( cutterAIdentifier == 0x01){ 
      autonomousNow = true; // autonomous mode becomes active - the cutter will make its own decisions
  ...
  if(millis() - cutStampA > CUT_INTERVAL && cutterOnA) cutResistorOffA(); // shuts the resistor off after the cut interval so it doesn't keep burning
